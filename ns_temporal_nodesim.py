...
class Graph():
	...
	def compute_edge_probs(self, temporal=False):
		'''
		...
		b=self.b	
		G = self.G
		sg_graph = StellarGraph.from_networkx(G)
		...
		for node in G.nodes():
			...
			if temporal:
				node_neighbours = [ngh for ngh, t in sg_graph.neighbors(node, include_edge_weight=True) if t > 20211108]
			
			for ngh in node_neighbours: #G.neighbors(node):
				nghbrs.append(ngh)
